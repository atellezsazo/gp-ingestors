...
// cleaning elements
const clean_elements = ['a', 'div', 'figure', 'h2', 'i', 'p', 'span', 'table', 'td', 'tr', 'ul'];

// delete attr (tag)
const remove_attr = ['align', 'class', 'data-field', 'data-original', 'h', 'height', 'id',
  'itemscope', 'itemprop', 'itemtype', 'photoid', 'rel', 'sizes', 'style',
  'title', 'type', 'w', 'width', 'dir',
];

// remove elements (body)
const remove_elements = ['.adtxt', '.auth', '.cl10', '.clr', '.cref', '.embedbox', '.fbshr', '.goosip',
  '.moreref', '.nsrc', '.taglst', '.tit', '#AbdVideoInPagePlayerWrapper', 'br',
...
function remove_empty_tag($, elem, tag='p') { // remove empty paragraphs
  $(elem).find(tag).get().map((t) => {
    const text=$(t).text(), child=$(t).children();
    if(child.length==0) $(t).remove();
    else if(child.length==1) {
      const ch = $(child[0]).children();
      if(text.length<=1 && ch.length==0) $(t).remove();
    }
  });
}
...
function ingest_article(hatch, uri) {
  return libingester.util.fetch_html(uri).then(($) => {
    ...
    body.find('span').get().map((span) => {
      ...
      if(text.includes('VIDEO:')) $(span).remove();
    });
