...
const remove_elements = ['#bx_polong', '#thepolong', '.detikads', '.news_tag',
                         '.newstag', '.thepolong', '[id^="beacon"]', 'a[href="#"]',
                         'script', 'video'];
...
function ingest_article(hatch, uri) {
    return libingester.util.fetch_html(uri).then(($profile) => {
        ...
        const art_thumbnail = $profile('meta[name="thumbnailUrl"]').attr('content');
        ...
        const art_uri = new URL(base_uri);
        const art_category = art_uri.host.split('.')[0];

        // Pull out the main image
        const $main_img = cheerio.load(`<img class="main_img" src="${art_main_img}">`);
        const asset_main_image = libingester.util.download_img($main_img('.main_img'), base_uri);
        asset_main_image.set_title(art_title);
