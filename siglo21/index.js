...
function ingest_profile(hatch, uri) {
    return libingester.util.fetch_html(uri).then(($profile) => {
        ...
        const modified_str = $profile('meta[property="og:updated_time"]').attr('content');
        const modified_date = new Date(Date.parse(modified_str));
        asset.set_last_modified_date(modified_date);

        // Set section name
        asset.set_section('News');
        ...
        const gallery = $profile('article.post .wp-caption img');

        // Pull out the article content
        ...
        body.find('h3').remove(); //title of embded content and blockquote
...
function main() {
    ...
    feed.load(news_list, function(err, rss){
         ...
         //   const news_uris =  rss.items.map((datum) => datum.url);
            ...
            // ingest_profile(hatch, "http://www.s21.gt/2017/03/ramos-apaga-infierno/");
            ingest_profile(hatch, "http://www.s21.gt/2017/02/inmobiliaria-habria-evadido-impuestos-q46-millones/");
            // ingest_profile(hatch, "http://www.s21.gt/2017/02/integrantes-de-los-48-cantones-de-totonicapan-bloquean-la-ruta-interamericana/");
