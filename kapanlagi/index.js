...
function ingest_article(hatch, uri) {
    return libingester.util.fetch_html(uri).then(($profile) => {
        if ($profile('meta[http-equiv="REFRESH"]').length == 1) throw 'Redirect';
        if ($profile('title').text().includes('404')) throw 404;
...
function main() {
    ...
    rp({ uri: RSS_URI, gzip: true }).then((res) => {
        ...
        parser.parseString(res, (err, result) => {
            ...
            Promise.all(links.map((link) => ingest_article(hatch, link, index++))).then(() => {
                return hatch.finish();
